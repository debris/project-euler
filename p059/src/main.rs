...
fn encrypt(text: &mut [u8], key: &[u8]){
    for (c, k) in text.iter_mut().zip(key.iter().cycle()) {
        *c ^= k;
    }
}

fn xor_decryption(text: &[u8], key_len: usize) -> u64 {
    let key = find_key(text, key_len);
    let mut text = text.to_vec();
    encrypt(&mut text, &key);
    text.into_iter().map(|x| x as u64).sum()
}

fn main() {
    println!("{}", xor_decryption(TEXT, 3));
}
